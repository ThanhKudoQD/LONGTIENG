...
export default function AudioList() {
  ...
  // PERF + Speed: tính effective wav_duration = wav_duration / speed
  // Audio block co/giãn theo speed lúc playback (audio.playbackRate).
  const withDur = useMemo(() =>
    withAudio.map(s => {
      const rawDur = s.wav_duration ?? (s.end_time - s.start_time)
      const speed = s.tts_speed != null ? s.tts_speed
                  : (s.character?.tts_speed != null ? s.character.tts_speed : 1.0)
      const effDur = rawDur / Math.max(0.5, Math.min(2.0, speed))
      return { ...s, wav_duration: effDur }
    }),
    [withAudio])
