...
// Form nhập alias cho 1 nhân vật
...
function AliasForm({ item, index, total, onConfirm, onSkip, onCancel }: AliasFormProps) {
  ...
  useEffect(() => {
    setAlias(item.role.character_name)
    setColor(CHAR_COLORS[index % CHAR_COLORS.length])
  }, [item, index])
    ...
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
      <div style={{ background: '#1a1a2e', borderRadius: 20, padding: 28, width: 460, border: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontSize: 17, fontWeight: 700, color: '#fff' }}>Đặt tên nhân vật trong phim</h3>
        ...
        <label style={{ fontSize: 12, color: '#9CA3AF', display: 'block', marginBottom: 10, fontWeight: 500 }}>Màu tag</label>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => onConfirm(alias.trim(), color)} disabled={!alias.trim()}
            ...
            {index + 1 < total ? 'Tiếp theo →' : 'Hoàn thành ✓'}
          </button>
          {total > 1 && (
...
export default function VoicePicker({ onSelect, onClose }: Props) {
  ...
  // Bắt đầu nhập alias cho các nhân vật đã chọn
  const startConfirm = () => {
    const items = cards.filter(c => selected.has(`${c.actor.id}-${c.role.id}`))
    if (!items.length) return
    // Thêm ngay không cần nhập alias — dùng character_name làm tên mặc định
    items.forEach((item, i) => {
      const color = CHAR_COLORS[i % CHAR_COLORS.length]
      onSelect(item.actor, item.role, item.role.character_name || item.actor.name, color)
    })
    setSelected(new Set())
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="rounded-2xl w-full max-w-6xl h-[92vh] flex flex-col overflow-hidden shadow-2xl relative"
        ...
        <div className="flex items-center justify-between px-8 py-5 flex-shrink-0"
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#9CA3AF', marginTop: 3 }}>
              Hiển thị <b style={{ color: '#fff' }}>{cards.length}</b> / {actors.reduce((s, a) => s + a.roles.length, 0)} vai
        ...
        {/* Filters */}
        <div className="px-8 py-4 flex-shrink-0 space-y-3"
          style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <input placeholder="🔍  Tìm tên diễn viên, nhân vật, phim..."
            value={search} onChange={e => setSearch(e.target.value)}
            style={{ width: '100%', padding: '10px 16px', borderRadius: 12, fontSize: 14, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', outline: 'none' }}
          />
              ...
              <div key={label} className="flex items-center gap-2">
                ...
                {Object.entries(opts).map(([k, v]) => (
                  <button key={k} onClick={() => set(k)}
                    style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s', background: val === k ? '#7C3AED' : 'rgba(255,255,255,0.07)', color: val === k ? '#fff' : '#9CA3AF', border: val === k ? '1px solid #7C3AED' : '1px solid rgba(255,255,255,0.1)' }}>
                    {v}
                  </button>
                ))}
              </div>
          ...
          <div className="flex items-center gap-2">
            <span style={{ fontSize: 12, color: '#6B7280', fontWeight: 500 }}>Sắp xếp:</span>
            {[['newest','Mới nhất'],['oldest','Cũ nhất'],['name','Tên A-Z'],['roles','Nhiều vai']].map(([v,label]) => (
              <button key={v} onClick={() => setSortBy(v)}
                style={{ padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: 'pointer', background: sortBy === v ? '#7C3AED' : 'rgba(255,255,255,0.07)', color: sortBy === v ? '#fff' : '#9CA3AF', border: `1px solid ${sortBy === v ? '#7C3AED' : 'rgba(255,255,255,0.1)'}` }}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          {loading ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: '#6B7280' }}>Đang tải...</div>
          ) : cards.length === 0 ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 200, color: '#6B7280' }}>Không tìm thấy</div>
          ) : (
              ...
              {cards.map(({ actor, role }) => {
                ...
                return (
                  <div key={key}
                    ...
                    onMouseEnter={e => { if (!isSelected) { (e.currentTarget as HTMLElement).style.border = '1px solid rgba(124,58,237,0.5)'; (e.currentTarget as HTMLElement).style.background = 'rgba(124,58,237,0.06)' } }}
                    onMouseLeave={e => { if (!isSelected) { (e.currentTarget as HTMLElement).style.border = '1px solid rgba(255,255,255,0.08)'; (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.04)' } }}>
                    ...
                    <div style={{ position: 'relative', height: 220, background: '#1a1a2e', overflow: 'hidden' }}>
                      ...
                      <div style={{ position: 'absolute', top: 8, right: 8, padding: '2px 8px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: actor.gender === 'nam' ? '#1D4ED8' : '#BE185D', color: '#fff' }}>
                        {actor.gender === 'nam' ? 'NAM' : 'NỮ'}
                      </div>
                      {role.lora_path && (
                        <div style={{ position: 'absolute', bottom: 8, left: 8, padding: '2px 8px', borderRadius: 20, fontSize: 10, fontWeight: 700, background: '#059669', color: '#fff' }}>LoRA ✓</div>
                      )}
                    </div>

                    {/* Info */}
                    <div style={{ padding: '12px 14px 14px' }}>
                      <p style={{ fontSize: 15, fontWeight: 700, marginBottom: 2 }}>{role.character_name}</p>
                      <p style={{ fontSize: 12, color: '#9CA3AF', marginBottom: 8 }}>{actor.name}</p>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                        <span style={{ padding: '3px 8px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: TYPE_COLORS[role.type] + '25', color: TYPE_COLORS[role.type] || '#9CA3AF', border: `1px solid ${TYPE_COLORS[role.type] + '40'}` }}>
                          {role.type === 'chinh' ? 'Vai chính' : role.type === 'phu' ? 'Vai phụ' : 'Phản diện'}
                        </span>
                        <span style={{ padding: '3px 8px', borderRadius: 6, fontSize: 11, background: 'rgba(255,255,255,0.07)', color: '#9CA3AF', border: '1px solid rgba(255,255,255,0.1)' }}>
                          {GENRES[role.genre] || role.genre}
                        </span>
                      </div>
                      <p style={{ fontSize: 11, color: '#6B7280', marginBottom: 8, fontStyle: 'italic' }}>{role.show_name}</p>
