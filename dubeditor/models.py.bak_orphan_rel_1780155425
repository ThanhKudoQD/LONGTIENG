...
class Project(Base):
    ...
    owner_id            = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)  # v5.0 user system
    last_export_config_json = Column(Text, nullable=True)   # v4.0 auto-save export config
...
# Đoạn append vào dubeditor/models.py
# ─── Export Video Models (v4.0) ───────────────────────────────────────────────
...
class ExportJob(Base):
    ...
    config_json   = Column(Text, nullable=False)       # JSON dump ExportConfig
    output_path   = Column(String, nullable=True)      # path tuyệt đối file output
    output_url    = Column(String, nullable=True)      # url /dub/exports/xxx.mp4
    output_size   = Column(Integer, default=0)         # bytes
    error_msg     = Column(Text, nullable=True)
    eta_sec       = Column(Float, default=0.0)
    created_at    = Column(DateTime(timezone=True), server_default=func.now())
...
class ExportPreset(Base):
    ...
    id          = Column(Integer, primary_key=True, index=True)
    name        = Column(String, nullable=False, unique=True)
    config_json = Column(Text, nullable=False)         # JSON dump partial ExportConfig
    created_at  = Column(DateTime(timezone=True), server_default=func.now())
    updated_at  = Column(DateTime(timezone=True), onupdate=func.now())
