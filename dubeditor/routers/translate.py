...
# v3.7.2: lưu cả asyncio.Task để cancel() ngắt ngay await call_llm.
# BackgroundTasks của FastAPI không trả Task → không cancel được.
...
async def _run_pipeline_background(pid: int, runner: TranslateRunner,
    ...
    try:
        ...
        # v3.7.2: có thể bị raise từ task.cancel() (httpx await bị ngắt) hoặc
        # từ self._check_cancelled(). Cả 2 cùng vào branch này.
        logger.info(f"[Pipeline pid={pid}] cancelled by user")
        try:
            runner._save_status("idle", 0.0, error="Cancelled by user")
            await _publish_progress(pid, "cancelled", 0.0, "Đã hủy pipeline")
        except Exception as ex:
            logger.warning(f"[Pipeline pid={pid}] cleanup after cancel failed: {ex}")
    except Exception as e:
...
async def start_translate(pid: int, req: TranslateStartRequest,
    ...
    # v3.7.2: dùng asyncio.create_task (KHÔNG dùng BackgroundTasks) để có
    # handle Task, cho phép cancel() ngắt await ngay lập tức (httpx, sleep…).
...
async def run_stage(pid: int, req: TranslateStageRequest,
    ...
    # v3.7.2: dùng asyncio.create_task để cancel được
    t = asyncio.create_task(task())
    _active_tasks[pid] = t
...
async def cancel_translate(pid: int):
    """Hủy pipeline đang chạy — ngắt ngay cả await call_llm.

    v3.7.2: ngoài set flag _cancelled (check giữa các stage), còn gọi
    task.cancel() để raise CancelledError vào trong await đang chờ HTTP
    response → httpx đóng connection ngay → pipeline dừng tức thì.
    """
    ...
    # 1. Set flag để các loop sync check
    if runner:
        runner.cancel()

    # 2. Cancel asyncio.Task để ngắt await ngay
    if task and not task.done():
        task.cancel()

    # 3. Emit progress event "cancelling" để UI cập nhật ngay
    try:
        await _publish_progress(pid, "cancelling", 0.0, "Đang hủy pipeline...")
    except Exception:
        pass

    return {"ok": True, "message": "Đã gửi tín hiệu hủy"}
